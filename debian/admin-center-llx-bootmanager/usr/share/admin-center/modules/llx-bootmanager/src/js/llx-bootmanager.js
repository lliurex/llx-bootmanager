...
LlxNetBootManagerClient.prototype.sortList = function(){
   ...
   if (self.BootOrder && self.AvailableList) {
     // Populate Lists
      ...
      console.log(self.BootOrder);
		 var itemclass="llx_bootmanager_ltsp";
		 ...
		 for (j in self.AvailableList) {
            ...
            console.log(boot_label.indexOf("menu label")+"***");
		 ...
		 var listitem=$(document.createElement("li")).html(boot_label).attr("id", self.BootOrder[i]).addClass("llx_bootmanager_menuEntry").addClass(itemclass);
	  ...
	  for (i in self.AvailableList) {
		 ...
		 var listitem=$(document.createElement("li")).html(self.AvailableList[i]['label']).attr("id", self.AvailableList[i]['id']).addClass("llx_bootmanager_menuEntry").addClass(itemclass);
		 // bootfromhd netinstall
...
LlxNetBootManagerClient.prototype.populateGui=function populateGui(){
   ...
   $("#llx_bootmanager_MainFrame").show();
      ...
      var n4dclass="LlxBootManager";
      var n4dmethod="setBootOrder";
      var arglist=new_boot_order;
      
      var time=$("#llx_bootmanager_slider")[0].noUiSlider.get();
   ...
   // Load Templates
    ...
    $.get("http://"+sessionStorage['server']+"/ipxeboot/getmenujson.php", function(data){
      ...
      credentials="";
         ...
         arglist=[];
          Utils.n4d(credentials, n4dclass, n4dmethod, arglist, function(response){
            //console.log("AAAAAAAAAAAAAAAAAAAAAAAAAAAAVVVVVVVVVVVVVVVVVVVVVVV"+response);
...
$(document).on("moduleLoaded", function(e, args){
    var moduleName="llx-bootmanager";
